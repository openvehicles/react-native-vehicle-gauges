...
import { GaugeFuelProps, GaugeColors, GaugeFonts } from '../types';
...
export const GaugeFuel: React.FC<GaugeFuelProps> = ({
  ...
  const generateTicks = () => {
    ...
    for (const level of majorTicks) {
      ...
      ticks.push(
        <Line
          key={`major-tick-${level}`}
          x1={x1}
          y1={y1}
          x2={x2}
          y2={y2}
          stroke={mergedColors.tickMajor}
          strokeWidth="2"
        />
      );
      ...
      let labelText = '';
      if (level === 0) labelText = 'E';
      else if (level === 25) labelText = '¼';
      else if (level === 50) labelText = '½';
      else if (level === 75) labelText = '¾';
      else if (level === 100) labelText = 'F';
    ...
    for (const level of minorTicks) {
      ...
      ticks.push(
        <Line
          key={`minor-tick-${level}`}
          x1={x1}
          y1={y1}
          x2={x2}
          y2={y2}
          stroke={mergedColors.tickMinor}
          strokeWidth="1"
        />
      );
    }
